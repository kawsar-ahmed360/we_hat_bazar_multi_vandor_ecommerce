@forelse(@$approve_request as $key=>$panding_re)
    <tr>
        <td style="font-size: 12px;">{{@$key+1}}</td>
        <td style="font-size: 12px;">{{@$panding_re->request_amoung}}</td>
        <td style="font-size: 12px;">{{@$panding_re->approve_amount??'null'}}</td>
        <td style="font-size: 12px;">{{@$panding_re->payment}}</td>
        <td style="font-size: 12px;"><span class="badge badge-success">Approve</span></td>
    </tr>
@empty
    <tr>
